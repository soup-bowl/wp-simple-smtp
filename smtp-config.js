...
function wpss_load_quicksettings( data ) {
	...
	// Description cell.
	selector_c1.outerHTML = "<th scope=\"row\">" + __( 'Quick Config', 'wp-simple-smtp' ) + "</th>";

	// Content cell.
	var datacount = data.configurations.length;
	options      += '<option>' + __( 'Select', 'wp-simple-smtp' ) + '</option>';
	...
	sel_warning.innerHTML = __( 'Automatically sets the default settings for most providers.', 'wp-simple-smtp' );
